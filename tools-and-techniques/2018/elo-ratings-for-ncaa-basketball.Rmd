...
Most, if not all of you have probably filled out an NCAA tournament bracket where the goal is to pick the winner of each game.  Filling out a bracket gives you a rooting interest in each game and is one of the reasons the tournament is so popular. The kaggle competition is slightly different from the typical tournament pool because instead of picking winners of games, the predictions are the probability team 1 will beat team 2.  So, instead of thinking deterministically, this competition forces you to think probabilistically.
...
load("~/git/ds/blog/tools-and-techniques/2018/data_skeptic.RData")
library(ggplot2)
library(dplyr)

probs <- seq(0.001, 0.999, 0.001)
outcomes <- data.frame(probs = probs, outcome = 1)

outcomes$log_loss <- -(1 * log(probs)) + (1 - 1) * (log(1 - probs))
p <- ggplot(outcomes, aes(probs, log_loss))
p + geom_point() +
  xlab("Predicted probability of team 1 winning") +
  ylab("Log loss when team 1 wins")
```
...
test_k %>% group_by(k) %>%
  filter(DayNum >= 133) %>%
  summarise(wp = mean(wteam_elo > lteam_elo)) %>%
  ggplot(aes(x = k, y = wp)) + geom_point(col = "blue") + 
  expand_limits(y = c(0.705,0.713)) +
  ylab("Values of k") +
  xlab("Winning Percentage") +
  ggtitle("Winning Percentages for Different Values of k") +
...
```{r, echo = FALSE, warning = FALSE, message = FALSE}
predictions %>%
  select(Team1_Name, Team1_elo) %>%
  distinct(Team1_Name, Team1_elo) %>%
  #bind_rows(xavier) %>%
  arrange(desc(Team1_elo)) %>%
  rename(Team = Team1_Name, ELO = Team1_elo) %>%
  head(10)
